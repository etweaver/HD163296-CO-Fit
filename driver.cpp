...
std::string getLocalTimestamp(){
	...
	char result[bufLen];
	strftime(result,bufLen,"%b %d %H:%M:%S %Y %Z",timeinfo);
	return result;
}
	...
	bool decideJump(std::uniform_real_distribution<double>& acceptDist, RNG& rng, ParameterSet& params){
		...
		for(int i=0; i<proposedCoords.size();i++){
			if(!params.inBounds(i,proposedCoords[i])){
				accept=false;
				std::cout << getLocalTimestamp() << "\tstep rejected: out of bounds on parameter " << i << std::endl;
			}
		}
		if(accept){
			std::cout << getLocalTimestamp() << "\tmove accepted: old value: " << currentValue << 
				", new value: " << proposedValue << ", logAcceptanceFactor: " << logAcceptanceFactor << std::endl;
...
public:
	...
	processor([this](){this->processResultsImpl();}){
		...
		generateWorkBlock();
		std::cout << getLocalTimestamp() << "\tfirst work block ready" << std::endl;
	}
...
protected:
	...
	void processResultsImpl(){
		while(!done()){
			...
			ensembleCounter++;
			std::cout << getLocalTimestamp() << "\tensemble counter: " << ensembleCounter.load() << std::endl;
			...
			if(ensembleCounter.load()==ensembleSize*nFrequencies){
				std::cout << getLocalTimestamp() << "\tprocessing finished ensemble" << std::endl;
				if(firstGeneration){
					for(int i=0;i<ensembleSize;i++){
				...
				ensembleCounter=0;
				std::cout << getLocalTimestamp() << "\tdone processing finished ensemble" << std::endl;
	...
	void generateWorkBlock(){
		std::cout << getLocalTimestamp() << "\tgenerating work block" << std::endl;
		...
		system("./calcTemp.sh");
		std::cout << getLocalTimestamp() << "\ttemperature model generated" << std::endl;
